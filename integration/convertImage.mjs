...
program.requiredOption('-i, --input <string>', 'ソースディレクトリ（必須）').requiredOption('-o, --out <string>', '出力先ディレクトリ（必須）').option('-w, --webp', 'webp化を行う', false).option('-a, --webp-suffix-add', 'webp化の際、拡張子を書き換え（false）するか追加（true）するか', false).option('-v, --svg', 'svgの最適化を行う', false).option('-z, --svgz', 'svgzを出力する', false).option('-n, --nosvg', 'svgzを出力した場合、svgは出力しない', false).option('-t, --truncate', '出力先のディレクトリを空にする', false).parse();
